  ...
  window.xgScanGalaxy = function() {
    ...
    for(var i = 2; i < nodes.childNodes.length-1; i+=2) {
      ...
      if(planet && planet.indexOf("Сканировать") >= 0) {
        ...
        $.get(url, function(row){ return function(data){
  ...
  function injectScaner() {
    if(document.getElementById("galaxy_form")) {
      ...
      $("#fleetstatusrow").parent().append(
        '<tr>\
            <th class="c" colspan="1"><span style="color: #CDB5CD">Целей:</span> ' + a + '</th>\
            <th class="c" colspan="1"><input type="submit" onclick="return xgScanGalaxy()" value="[ Сканировать систему ]"></th>\
            <th class="c" colspan="1"><span style="color: #CDB5CD">Стоимость:</span> ' + gap(a*5000) + '</th>\
        </tr>');
    }
  }
